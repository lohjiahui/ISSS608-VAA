...
pacman::p_load(ggplot2, tidyverse, ggrepel, patchwork, 
               ...
               gganimate, crosstalk, ggstatsplot, dplyr, lubridate, ggnewscale, broom, knitr, ggridges, viridis, ggdist)
```

The *Participants.csv* and *FinancialJournal.csv* data is then importanted from csv using `readr::read_csv()` and saved under *"Participants"* and *"Finance"*, respectively.
...
-   In order to have the flexibility to analyse the age variable as a band, we will also re-code the age variable into 10 year bands. The new variable is saved under *age_band*.
...
-   1,113 duplicated rows were also observed in the *Finance* dataset. Using `distinct()` these rows should be removed from subsequent analyses. The truncated dataset was saved under *fin_new.*
...
-   Under the amount variable in *FinancialJournal*, inflow and outflow of money transacted is recorded by positive and negative numbers, respectively. This may cause confusion when visualising the data. We will process the data and use the absolute values using `abs()`.
...
-   Before merging, we will create the following new variables using `mutate()`:
    ...
    -   Total Savings, across months e.g., TotSav_Mar
...
As a next step, let's explore whether there is a relationship between age and education. Since the distribution of age departed from normality, we will use a non-parametric test. Looking at the boxplots, it was observed that there was no statistically significant correlations between the two variables.
...
From the line charts below, we can see that monthly expenditure among residents tended to be fairly stable across months, regardless of their education levels. Even in the month of March, where earnings tended to spike, expenditure remained fairly low. Notably, those who earned more tended to have sizable monthly savings.

::: callout-note
This suggests that residents are generally frugal with their money and spend within their means. A good sign in terms of financial health!
:::
...
To better undertand spending patterns, new variables were calculated to derive the average monthly spending on each category i.e., education, food, recreational, shelter.
...
Breaking down this relationship further by interest groups, we also observe that this positive correlation is true, across all groups. Additional analyses of interest groups by size i.e., number of interested residents, and joviality scores recorded no significant differences.
