...
The code chunk below will group the rows by unique 'id'. Specifically, for the variable type, each additional value will be recorded in a new column e.g. "type_2". For the remaining variables, values will concatenated within the original columns, and seperated by a comma. Odd values like character(0), and Unknown will also be changed to 'NA'.
...
words_to_include <- c("fish", "seafood", "fresh", "shrimp", "prawn", "salmon", "tuna", "shellfish", "crabs", "squid", "mollusks", "crustaceans", "lobster", "oysters", "clams", "bass", "trout", "tuna", "cod", "shark", "chum", "sea", "ocean", "aquatic", "shipping", "shipments", "shipments", "pier")

nodes_grouped$product_services <- tolower(nodes_grouped$product_services)

nodes_grouped <- nodes_grouped %>%
  mutate(category = case_when(
    !is.na(product_services) & grepl(paste("\\b", words_to_include, "\\b", collapse = "|"), tolower(product_services)) ~ "Fishing Activities",
    !is.na(product_services) ~ "Non-Fishing Activities",
    TRUE ~ NA_character_
  ))
...
-   The `grepl()` function is used to check if the 'product_services' column contains any of the words in the 'words_to_include' list, ignoring case sensitivity.
-   Next the `paste(words_to_include, collapse = "|")` portion of the code is used to create a regular expression pattern that matches any of the words in the list.
...
Zooming only into nodes that were companies, we see that 410 were associated with the fishing industry, while the rest were either unlikely to be associated, or unknown.
...
### 5.1 Reviewing Networks by Diameter

#### 5.1.1 Count and Size distribution

Network diameter refers to the shortest distance between the two modes distant nodes in network.

Using the `components()` function in igraph, we see that 391 sub-graphs make up the whole network. The number of nodes in each sub-graph were also counted. Both set of values were added as new variables in the 'nodes_fishing' dataframe.

Looking at the 2 histograms below, we see that majority of the nodes being to fairly small sub-networks (2 nodes were the mode), and of diameter less than 4.
...
histogram <- ggplot(nodes_fishing, aes(x = num_nodes)) +
  ...
  labs(x = "Number of Nodes", y = "Count", title = "Distribution of Number of Nodes within sub-graph") +
  theme_classic()
...
While we cannot assume illegal activities, it may be wise for FishEye to take a closer look at these businesses to understand how they keep operations so lean and yet yield such high returns. Specifically, they can start with the 3 identified companies first.
...
### 5.2 Inspecting larget sub-networks

Aside from looking at small companies with large earnings, we will also look at the flip side i.e., exceptionally large networks, to take a closer look at their operations. From the scatterplot above, the companies with the top 3 highest number of nodes in their networks were:
...
#### 5.2.1 Congo Rapids Ltd Corperation - Ego Network
...
#### 5.2.1 BlueWater Bistro GmbH Industrial & Aqua Aura SE Marine LifeCongo Rapids Ltd Corperation - Sub-network

Unlike the network graph for Congo Rapids Ltd Corperation, this network subgraph is slightly more interesting. Firstly, we see one individual, James Martinez, serving as a bridge between the two firms, with Aqua Aura being the larger firm between the two.
...
It is interesting that James Martinez, a beneficial owner from such a large firm, would also be associated with a much smaller firm, and from a totally different country. It may be useful for FishEye to look into such individuals as well, as they may be key bridges between legal and illegal entities.
:::
...
### 5.3 Individuals associated with multiple companies 

Earlier when we were discussing the nature of the relationships provided in the edges file, one of the key ideas was to look at influencial individuals in the system. These individuals may be like James Martinez, playing a bridging role between and large and small firm. But let's look at it from a different perpective. Let's identify the individual who is associated with the most firms. 
...
This trend has some similarly with the one above, where one individual serves as a bridge between large and small firms. What is slightly more suspicious in the case of James Brown is that one of the company is owned soly by him. 
